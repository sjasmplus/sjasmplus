...
def lit_y(tb: TestBits) -> T:
    templ_add_extra(tb, """
        auto Y = GENERATE(range(0, 8));
    """)
    tb.subst['asm'] += '" " + std::to_string(Y) + ","'
    tb.mcbits[0] = wrap_uint8_t(byte_to_hex(tb.mcbits[0]) + ' | (Y << 3)')
    return T(tb)
...
for x in range(4):
    match x:
        case 0:
            cb('ROT r', '') >> sx(x) >> rot >> r_z >> done
        case 1:
            cb('BIT y, r', 'BIT') >> sx(x) >> lit_y >> r_z >> done
        case 2:
            cb('RES y, r', 'RES') >> sx(x) >> lit_y >> r_z >> done
        case 3:
