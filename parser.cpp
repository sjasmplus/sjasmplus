...
int ParseExpPrim(char*& p, aint& nval) {
	...
	if (*p == '(') {
		++p; res = ParseExpression(p, nval); if (!need(p, ')')) {
...
char* ReplaceDefine(char* lp) {
	...
	while ('o') {
		...
		if (DefineTable.DefArrayList) {
			CStringList* a = DefineTable.DefArrayList;
			aint val;
			//cout << lp << endl;
			while (*(lp++) && (*lp <= ' ' || *lp == '['));
			//cout << lp << endl;
...
void ParseLabel() {
	...
	*tp = 0;
	...
	if (isdigit((unsigned char) * tp)) {
		...
		val = atoi(tp); 
		//cout << CurrentLine << " " << val << " " << CurAddress << endl;
		if (pass == 1) {
		...
		if (pass == LASTPASS) {
			...
			if (!IsDEFL && val != oval) {
				...
				//cout << "" << filename << ":" << CurrentLocalLine << ":(DEBUG)  " << "Label has different value in pass 2: ";
	  			//cout << val << "!=" << oval << endl;
