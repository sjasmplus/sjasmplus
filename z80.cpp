...
namespace Z80 {
	...
	int GetByte(char*& p) {
		...
		if (!ParseExpression(p, val)) {
			Error("Operand expected", NULL); return 0;
		}
		check8(val);
		return val & 255;
	}
	...
	void OpCode_DJNZ() {
		...
		do {
			...
			if (jmp < -128 || jmp > 127) {
				char el[LINEMAX];
				SPRINTF1(el, LINEMAX, "Target out of range (%i)", jmp);
				Error(el, 0); jmp = 0;
			}
	...
	void OpCode_JR() {
		...
		do {
			...
			case Z80C_PE:
			case Z80C_PO:
				Error("Illegal condition", 0); break;
			default:
				e[0] = 0x18; break;
			}
		...
		do {
			...
			case Z80_E:
				...
				case Z80_L:
					e[0] = 0x58 + reg; break;
				case Z80_IXL:
					e[0] = 0xdd; e[1] = 0x5d; break;
				case Z80_IXH:
					e[0] = 0xdd; e[1] = 0x5c; break;
				case Z80_IYL:
					e[0] = 0xfd; e[1] = 0x5d; break;
				case Z80_IYH:
					e[0] = 0xfd; e[1] = 0x5c; break;
				default:
					if (oparen(lp, '[')) {
						if ((reg = GetRegister(lp)) == Z80_UNK) {
							break;
						...
						if ((reg = GetRegister(lp)) == Z80_UNK) {
							--lp; e[0] = 0x1e; e[1] = GetByte(lp); break;
						}
					} else {
						e[0] = 0x1e; e[1] = GetByte(lp); break;
					}
					switch (reg) {
					case Z80_HL:
						if (cparen(lp)) {
					...
					case Z80_IX:
					case Z80_IY:
						e[1] = 0x5e; e[2] = z80GetIDxoffset(lp); if (cparen(lp)) {
																 	e[0] = reg;
																 } break;
					default:
						break;
			...
			case Z80_H:
				...
				default:
					if (oparen(lp, '[')) {
						...
						if ((reg = GetRegister(lp)) == Z80_UNK) {
							--lp; e[0] = 0x26; e[1] = GetByte(lp); break;
						}
					} else {
						e[0] = 0x26; e[1] = GetByte(lp); break;
					}
					switch (reg) {
					case Z80_HL:
						if (cparen(lp)) {
					...
					case Z80_IX:
					case Z80_IY:
						e[1] = 0x66; e[2] = z80GetIDxoffset(lp); if (cparen(lp)) {
																 	e[0] = reg;
																 } break;
					default:
						break;
			...
			case Z80_BC:
				...
				default:
					...
					case Z80_HL:
						if (cparen(lp)) {
							e[0] = 0x4e;
						} e[1] = 0x23; e[2] = 0x46; e[3] = 0x2b; break;
					case Z80_IX:
					case Z80_IY:
			...
			case Z80_DE:
				...
				default:
					if (oparen(lp, '[')) {
						if ((reg = GetRegister(lp)) == Z80_UNK) {
							b = GetWord(lp); e[1] = 0x5b; e[2] = b & 255; e[3] = (b >> 8) & 255; if (cparen(lp)) {
																								 	e[0] = 0xed;
																								 } break;
						}
					} else {
				...
				case Z80_IY:
					...
					if (comma(lp)) {
						...
						case Z80_A:
							...
							break;
						default:
							Error("Illegal operand", lp, SUPPRESS);
						}
					}
					break;
				default:
	...
	void OpCode_RST() {
		...
		do {
			...
			case 0x38:
				e = 0xff; break;
			default:
				Error("Illegal operand", line); *lp = 0; return;
			}
			EmitByte(e);
			/* (begin add) */
			if (*lp && comma(lp)) {
