...
int cmphstr(char *&p1, char *p2) {
  ...
  if (isupper(*p1)) {
    while (p2[i]) { 
      if (p1[i]!=toupper(p2[i])) v=0;
      else ++v; 
      ++i; 
    }
    if (strlen(p2) != v) return 0;
  } else {
    while (p2[i]) { 
      if (p1[i]!=p2[i]) v=0; 
      else ++v; 
  ...
  if (i<=strlen(p1) && p1[i]>' '/* && p1[i]!=':'*/) return 0;
  p1+=i;
  return 1;
  } else return 0;
}
...
char *getfilename(char *&p) {
  ...
  *fn=0;
  ...
  if (*p && strstr(p,":")) { o2=1; } /* added */
  /* while (!white() && *p!='"' && *p!='>') { *np=*p; ++np; ++p; } */
  while (*p && *p!='"' && *p!='>' && !(o==0 && o2==1 && *p==':')) { *np=*p; ++np; ++p; }
  if (*p && o==1) if (*p=='"') ++p; else error("No closing '\"'",0);
  else if (*p && o==2 && *p!='>') error("No closing '>'",0);
  else if (*p) ++p;
  *np=0; 
  for(np=fn;*np;++np) {
#ifdef WIN32
...
char *gethobetaname(char *&p) {
  ...
  *fn=0;
  ...
  if (*p && strstr(p,":")) { o2=1; } /* added */
  while (*p && !white() && *p!='"' && *p!='>' && !(o==0 && o2==1 && *p==':')) { *np=*p; ++np; ++p; }
  if (*p && o==1) if (*p=='"') ++p; else error("No closing '\"'",0);
  else if (*p && o==2 && *p!='>') error("No closing '>'",0);
  else if (*p) ++p;
  *np=0; 
