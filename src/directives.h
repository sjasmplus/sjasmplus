...
void insertDirectives(AsmTy &Asm);

template<typename AsmTy>
bool findDirective(const std::string &Name);

template<typename AsmTy>
bool applyDirIfExists(const std::string &Name, bool BOL = false);

template<typename AsmTy>
bool applyDupDirIfExists(const std::string &Name, bool BOL = false);

void checkRepeatStackAtEOF();

template<typename AsmTy>
optional<std::string> doSAVETRD(AsmTy &Asm, const fs::path &FileName, const zx::trd::HobetaFilename &HobetaFileName,
                                int Start, int Length, int Autostart) {
    std::vector<uint8_t> Data;
    Data.resize(Length);
    readRAM(Asm, Data.data(), Start, Length);
    return addFile(Data, FileName, HobetaFileName, Start, Length, Autostart);
}

template<typename AsmTy>
optional<std::string> doSAVESNA(AsmTy &Asm, const fs::path &FileName, uint16_t Start) {
...
struct Directives {
    ...
    inline static FunctionTable DirectivesTable = {};
    inline static FunctionTable DirectivesTable_dup = {};

    // May be defined by the END directive and used by SAVESNA & SAVETAP
    ...
    static void dirALIGN() {
        ...
        switch (val) {
            ...
            case 16384:
            case 32768: {
                optional<uint8_t> FillByte = std::nullopt;
                if (!noexp && comma(lp)) {
...
// Do not process beyond the END directive
    ...
    static void dirSAVETRD() {
        ...
        if (exec) {
            auto Err = doSAVETRD(Asm, FileName, HobetaFileName, start, length, autostart);
            if (Err) {
                Fatal(*Err);
            }
        }
...
//    char size = 0;
    ...
    static void insertDirectives(AsmTy &Asm) {
...
template<typename AsmTy>
void insertDirectives(AsmTy &Asm) {
    Directives<AsmTy>::insertDirectives(Asm);
}

template<typename AsmTy>
bool findDirective(const std::string &Name) {
    return Directives<AsmTy>::DirectivesTable.find(Name);
}

template<typename AsmTy>
bool applyDirIfExists(const std::string &Name, bool BOL) {
    return Directives<AsmTy>::DirectivesTable.callIfExists(Name, BOL);
}

template<typename AsmTy>
bool applyDupDirIfExists(const std::string &Name, bool BOL) {
    return Directives<AsmTy>::DirectivesTable_dup.callIfExists(Name, BOL);
}
