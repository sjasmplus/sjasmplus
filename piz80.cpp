...
Z80Reg getz80reg(char *&p) {
  ...
  case 'h':
    /* (begin add) */
    if (*p=='x') {
      if (!islabchar(*(p+1))) { ++p; return Z80_IXH; }
    }
    if (*p=='y') {
      if (!islabchar(*(p+1))) { ++p; return Z80_IYH; }
    }
    /* (end add) */
  ...
  case 'y':
    ...
    if (*p=='l') {
      if (!islabchar(*(p+1))) { ++p; return Z80_IYL; }
    }
    break;
  case 'x':
    if (*p=='h') {
    ...
    if (*p=='l') {
      if (!islabchar(*(p+1))) { ++p; return Z80_IXL; }
    }
    break;
  /* (end add) */
  case 'l':
...
void pizADC() {
  ...
  EmitBytes(e);
  /* (begin add) */
  if (*lp && comma(lp)) {continue;} else break;
  } while ('o');
  /* (end add) */
}
...
void pizMULUB() {
  ...
  switch (reg) {
    case Z80_B: e[0]=0xed; e[1]=0xc5; break;
    case Z80_C: e[0]=0xed; e[1]=0xcd; break;
    case Z80_D: e[0]=0xed; e[1]=0xd5; break;
    case Z80_E: e[0]=0xed; e[1]=0xdd; break;
    default: ;
  }
  EmitBytes(e);
}
...
void pizMULUW() {
  ...
  switch (reg) {
    case Z80_BC: e[0]=0xed; e[1]=0xc3; break;
    case Z80_SP: e[0]=0xed; e[1]=0xf3; break;
    default: ;
  }
  EmitBytes(e);
}
...
void InitpiZ80() {
  ...
  z80funtab.insert("exa",pizEXA); /* added */
  z80funtab.insert("exd",pizEXD); /* added */
  ...
  if (popreverse) z80funtab.insert("pop",pizPOPreverse); else z80funtab.insert("pop",pizPOP);
